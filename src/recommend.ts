...
export function defenderFrom(slug: string, gear: GearState, kit: KitOption | null, tier: Tier = '0'): Defender {
  const base = charBySlug[slug]
  const g = gearStats(slug, gear, tier)
  return {
...
export function hpFrom(slug: string, gear: GearState, kit: KitOption | null = null, tier: Tier = '0'): number {
  const base = charBySlug[slug]
  const g = gearStats(slug, gear, tier)
...
export type Target =
  ...
  | { kind: 'field'; items: { w: number; def: Defender; hp: number }[]; totalW: number }
...
export function subFieldTarget(kind: FieldKind, stage: number, tier: Tier = '0', excludeSlug?: string): Target {
  ...
  const items: { w: number; def: Defender; hp: number }[] = []
  for (const c of characters) {
    ...
    if (!wantTank) {
      const kit = m.defenseKits?.[0] ?? null
      items.push({ w: m.pickRate, def: defenderFrom(c.slug, gear, kit, tier), hp: hpFrom(c.slug, gear, kit, tier) })
      continue
    }
    const dks = m.defenseKits?.length ? m.defenseKits : []
    if (!dks.length) {
      if (kind === 'tankArmor') items.push({ w: m.pickRate, def: defenderFrom(c.slug, gear, null, tier), hp: hpFrom(c.slug, gear, null, tier) })
      continue
    }
    ...
    for (const kit of dks) {
      ...
      if (w > 0) items.push({ w, def: defenderFrom(c.slug, gear, kit, tier), hp: hpFrom(c.slug, gear, kit, tier) })
    }
  }
