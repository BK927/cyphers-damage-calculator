...
- **킬 좌표 = 처치 시점의 킬러 본인 위치** (피격자 위치 아님 — 2026-07-14 검증). 근거: 같은 매치의 대인킬 24건=데스 24건(동일 사건)인데 좌표는 4/24만 일치, 킬→최근접 데스 거리 중앙값 10px, 원거리 캐릭터(클레어) 킬 3건이 매치 최장거리 3개(26~46px)=사거리만큼 이격. `die` = 본인 사망 위치.
- **좌표계**: 맵 이미지 `record-cyphers.neople.co.kr/images/map/{mapId}.png` 픽셀 기준(맵은 세로형 ≈324×512). `count`=그 지점 이벤트 수.
